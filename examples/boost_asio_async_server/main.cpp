...
class tcp_server {
public:
    tcp_server(boost::asio::io_service &io_service, string const &address, unsigned short port)
        : acceptor_(io_service, tcp::endpoint(boost::asio::ip::address::from_string(address), port)) {
...
private:
    void start_accept() {
        tcp_connection::pointer new_connection = tcp_connection::create(
                (boost::asio::io_service &) acceptor_.get_executor().context());
